...
const clients = [{
  ...
  logo: '/images/clients/zhukovskiy.png',
  hasLogo: true,
}, {
  name: 'ООО «СИМПЛ ФУД»',
  description: 'Кейтеринговая компания',
  logo: '/images/clients/simple-food.png',
  hasLogo: true,
}];
        ...
        <SliderContainer>
          <SliderTrack>
            {doubledClients.map((client, index) => (<LogoCard key={index}>
              <LogoWrapper>
                {client.hasLogo ? (<LogoImage src={client.logo} alt={client.name}/>) : (
                  <LogoPlaceholder>{client.name.substring(0, 3)}</LogoPlaceholder>)}
              </LogoWrapper>
